...
export async function saveRecordingsToDatabase(rowCount: number, recordings: Map<string, Recording[]>) {
  ...
  try {
    ...
    const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/plinko/recordings`, {
      method: 'POST',
      headers: {
...
export async function loadRecordingsFromDatabase(rowCount: number): Promise<Map<string, Recording[]> | null> {
  ...
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/plinko/recordings/${rowCount}`);
