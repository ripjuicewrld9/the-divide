      ...
      const revealNextCase = async (caseIndex) => {
        ...
        if (reelRef.current) {
            ...
            onStart: () => {
              ...
              setTimeout(() => {
                const tickInterval = setInterval(() => {
                  ...
                  // Play ticking sound using click.wav
                  const tickAudio = new window.Audio('/sounds/click.wav');
                  tickAudio.volume = 0.15;
                  tickAudio.play().catch(e => console.warn('Tick sound failed:', e));
                  tickCount++;
                }, 200); // Every 200ms
          ...
          spinTimeline.eventCallback('onComplete', () => {
            ...
            // Play landing sound
            // ...existing code... (landing sound now uses coin.wav/gold.wav)
            ...
            if (isGoldSpinItem) {
              ...
              // Skip gold spin if there's only 1 gold item (nothing to spin for)
              if (goldItems.length <= 1) {
                console.log('[Reel] Only 1 gold item, skipping secondary spin');

                // Play gold sound immediately
                const goldAudio = new window.Audio('/sounds/gold.wav');
                goldAudio.volume = 0.5;
                goldAudio.play().catch(e => console.warn('Gold sound failed:', e));

                // Show reveal panel immediately
                if (revealName && revealValue && revealPanel) {
                  revealName.textContent = drawnItem.name || 'Unknown Item';
                  revealValue.textContent = `$${caseValue || drawnItem.value || 0}`;
                  requestAnimationFrame(() => requestAnimationFrame(() => {
                    revealPanel.classList.add('show');
                  }));
                }
                ...
                // Multiple gold items - do the full gold spin animation
                ...
                setTimeout(async () => {
                  ...
                  gsap.to(reelRef.current, {
                    ...
                    onComplete: () => {
                      ...
                      // ...existing code... (gold spin sound now uses gold.wav)
              ...
              // Play landing sound
              const landingAudio = new window.Audio('/sounds/ding.wav');
              landingAudio.volume = 0.3;
              landingAudio.play().catch(e => console.warn('Landing sound failed:', e));
...
export default function CaseBattleArenaReelNative({ battleData, onBattleComplete }) {
        ...
        return new Promise((resolve) => {
          ...
          spinTimeline.to(reelRef, {
            ...
            onComplete: () => {
              ...
              // Play ding.wav for normal, gold.wav for goldspin
              if (drawnItem.rarity === 'legendary' || drawnItem.rarity === 'epic') {
                const goldAudio = new window.Audio('/sounds/gold.wav');
                goldAudio.volume = 0.5;
                goldAudio.play().catch(e => console.warn('Gold sound failed:', e));
              } else {
                const dingAudio = new window.Audio('/sounds/ding.wav');
                dingAudio.volume = 0.25;
                dingAudio.play().catch(e => console.warn('Ding sound failed:', e));
              }
            }
          });
  ...
  return (
    <div className="arena-wrapper">
      ...
      {/* ...existing code... (debug overlay removed) */}
      {/* Winners overlay only if battle ended */}
      {battleEnded ? (
      ...
      <div className="unified-arena-container">
        {!battleEnded ? (
          battleData?.mode === 'group' ? (
            /* Group Mode - All players together, no teams */
            <div className="unified-team-group" style={{ width: '100%' }}>
            ...
            /* Normal/Crazy Mode - Team vs Team */
            <>
              {/* Team A */}
      ...
      {!battleEnded && (
        <div className="battle-footer">
          {battleData?.mode === 'group' ? (
            /* Group Mode - All players in shared pot, no teams */
            <div className="battle-footer-content" style={{ width: '100%' }}>
              {players
                .filter(player => !player.id.startsWith('empty-'))
                .map(player => (
                  <div key={player.id} className="battle-footer-player">
                    <div className="footer-player-info">
                      ...
                      <div className="footer-player-details">
                        ...
                        <p className="footer-player-team">Shared Pot</p>
                      </div>
                    </div>
