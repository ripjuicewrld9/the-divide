...
export default function DiscordOAuthButton() {
  ...
  const handleDiscordLink = () => {
    ...
    if (token) {
      sessionStorage.setItem('discord_link_token', token);
    }
  ...
  const handleDiscordUnlink = async () => {
    ...
    try {
      ...
      if (response.ok) {
        updateUser({ discordId: '', discordUsername: '' });
      }
    } catch (error) {
    ...
    <div className="bg-[#1a1d29] rounded-lg p-6 border border-gray-700">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-400">
            {user?.discordId 
              ? `Linked as ${user.discordUsername || user.discordId}`
              : 'Connect your Discord account for support ticket notifications'
            }
          </p>
        </div>
      </div>
      ...
      {user?.discordId ? (
        <div className="space-y-3">
          <div className="bg-green-500/10 border border-green-500/50 rounded-lg p-3">
            <p className="text-sm text-green-400">
              ✅ Discord connected! You'll be mentioned in support ticket threads.
            </p>
          </div>
          <button
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full px-4 py-3 bg-[#5865F2] hover:bg-[#4752C4] disabled:bg-gray-600 text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
        >
          {isLoading ? (
            'Redirecting...'
          ) : (
