/**
 * Casino Simulation Script
 * Tests overall profitability across all games with realistic betting patterns
 */

import { paytables } from '../paytable-data.js';

// Simulation parameters
const TOTAL_BETS = 100000; // Total number of bets across all games
...
// PLINKO CAPS
const MAX_PLINKO_BET = 200;
const MAX_PLINKO_WIN = 200000;

// KENO CAPS
const MAX_KENO_BET = 200;
const MAX_KENO_WIN = 200000;

// Game simulation functions
function simulatePlinko(betAmount, riskLevel) {
  // Cap bet amount
  betAmount = Math.min(betAmount, MAX_PLINKO_BET);
  ...
  // Get multiplier for this bin - NO CAP, allow full multipliers
  const multiplier = getPlinkoMultiplier(binIndex, rowCount, riskLevel);

  // Calculate payout with max win cap
  let payout = betAmount * multiplier;
  if (payout > MAX_PLINKO_WIN) {
    payout = MAX_PLINKO_WIN;
  }
  ...
  return {
    ...
    details: { binIndex, riskLevel, rowCount, capped: payout === MAX_PLINKO_WIN }
  };
}

// Simplified version of applyBinBias for simulation
function applyBinBiasSimple(randomValue, rowCount) {
  const HOUSE_EDGE_BIAS = 0.99; // Match the bias in plinkoProofOfFair.js
...
function simulateBlackjack(betAmount) {
  ...
  // Real blackjack is complex, but we'll simulate the key outcomes
...
// Main simulation
function runCasinoSimulation() {
  console.log('🎰 Running Casino Profitability Simulation...\n');
  ...
  console.log(`Plinko: Max Bet $${MAX_PLINKO_BET}, Max Win $${MAX_PLINKO_WIN.toLocaleString()}, 95% Bias`);
  ...
  for (let i = 0; i < TOTAL_BETS; i++) {
    ...
    // Progress indicator
    if ((i + 1) % 10000 === 0) {
      console.log(`Progress: ${((i + 1) / TOTAL_BETS * 100).toFixed(1)}% - Current Profit: $${totalProfit.toFixed(2)}`);
    }
  }

  // Calculate results
  const houseEdge = (totalProfit / totalWagered) * 100;
  const rtp = ((totalPaidOut / totalWagered) * 100);

  console.log('\n' + '='.repeat(60));
  console.log('🎰 CASINO SIMULATION RESULTS');
  console.log('='.repeat(60));

  console.log(`\n💰 OVERALL PERFORMANCE:`);
  console.log(`Total Wagered:    $${totalWagered.toLocaleString()}`);
  console.log(`Total Paid Out:   $${totalPaidOut.toLocaleString()}`);
  console.log(`House Profit:     $${totalProfit.toLocaleString()}`);
  ...
  for (const [gameName, stats] of Object.entries(gameStats)) {
    ...
    console.log(`  Bets: ${stats.bets.toLocaleString()} (${(stats.bets / TOTAL_BETS * 100).toFixed(1)}%)`);
    console.log(`  Wagered: $${stats.wagered.toLocaleString()}`);
    console.log(`  Paid Out: $${stats.paidOut.toLocaleString()}`);
  ...
  console.log(`\n🚀 BIG WINS (100x+ multiplier): ${bigWins.length}`);
  if (bigWins.length > 0) {
    console.log(`Biggest Win: ${Math.max(...bigWins.map(w => w.multiplier)).toFixed(1)}x ($${Math.max(...bigWins.map(w => w.payout)).toLocaleString()})`);
    const bigWinValue = bigWins.reduce((sum, w) => sum + w.payout, 0);
    console.log(`Total Big Win Payouts: $${bigWinValue.toLocaleString()}`);
  }

  console.log(`\n📊 PROFITABILITY ANALYSIS:`);
  if (houseEdge > 0) {
    console.log(`✅ PROFITABLE: You'd make $${totalProfit.toLocaleString()} on $${totalWagered.toLocaleString()} in action`);
    const dailyVolume = 10000; // Assume $10k daily volume
    const projectedDailyProfit = dailyVolume * (houseEdge / 100);
    ...
    console.log(`❌ UNPROFITABLE: You'd lose $${Math.abs(totalProfit).toLocaleString()} on this volume`);
  }
